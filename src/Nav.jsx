...
import bar from "./assets/images/icon-menu.svg";

export default function Nav() {
  const [open, setopen] = useState(true);

  return (
    <nav role="nav" className="flex h-20  flex-row justify-between px-10 pt-7 relative">
      <div className="">
      ...
      <ul className="hidden md:flex flex-row gap-7 font-Inter">
      ...
      <div className="md:hidden cursor-pointer ">
        {open && <img src={bar} alt="bar" onClick={() => setopen(!open)} />}

        {!open && (
          // <div className="absolute z-100  w-[200px] h-[80vh] bg-red-400">
          <div className="top-0 right-0 w-[55vw] bg-white text-gray-700 fixed h-full">
            <div className="w-full flex justify-end p-9">
              <img src={cross} alt="cross" onClick={() => setopen(!open)} />
            </div>

            <ul className="flex flex-col pl-8 font-Inter text-lg font-semibold pt-9">
              <li className="hover:text-SoftRed cursor-pointer pb-4">Home</li>
              <li className="hover:text-SoftRed cursor-pointer pb-4">New</li>
              <li className="hover:text-SoftRed cursor-pointer pb-4">Popular</li>
              <li className="hover:text-SoftRed cursor-pointer pb-4">Trending</li>
              <li className="hover:text-SoftRed cursor-pointer">Categories</li>
            </ul>
          </div>
        )}
